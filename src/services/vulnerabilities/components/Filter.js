      ...
      <Toolbar className={classes.mt3}>
        <div className={classes.search}>
          <InputBase
            ...
            onKeyPress={(ev) => {
              if (ev.key === 'Enter') {
                history.push(`/vulnerabilities/q=${ev.target.value}`);
        ...
        >
          <MenuItem onClick={async () => {
            history.push("/vulnerabilities/a=Finished");
            dispatch(invalidateContracts());
          }}>Analyses: Finished</MenuItem>
          <MenuItem onClick={async () => {
            history.push("/vulnerabilities/a=Error");
            dispatch(invalidateContracts());
          }}>Analyses: Error</MenuItem>
          <Divider />
          <MenuItem onClick={async () => {
            history.push("/vulnerabilities/s=Low");
            dispatch(invalidateContracts());
          }}>Severity: Low</MenuItem>
          <MenuItem onClick={async () => {
            history.push("/vulnerabilities/s=Medium");
            dispatch(invalidateContracts());
          }}>Severity: Medium</MenuItem>
          <MenuItem onClick={async () => {
            history.push("/vulnerabilities/s=High");
            dispatch(invalidateContracts());
...
export default compose(
  connect(state => {
    const { isFiltered } = state.vulnerabilities || { isFiltered: false };
    return { isFiltered };
  }),
  withStyles(styles)
)(withRouter(Filter));